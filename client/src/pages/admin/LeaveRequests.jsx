          ...
          {[
            ...
            >
              ...
              <span className={`px-1.5 py-0.5 rounded-md text-[10px] ${
                filterStatus === btn.id ? "bg-white/50" : "bg-slate-100"
              }`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mb-12">
          {filteredLeaves.map((leave) => (
            <div key={leave._id} className="bg-white rounded-[32px] shadow-sm border border-slate-100 p-7 flex flex-col hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-500 group relative overflow-hidden">
              ...
              <div className={`absolute top-0 right-0 w-32 h-32 -mr-16 -mt-16 rounded-full opacity-[0.03] transition-all duration-500 group-hover:scale-150 ${
                leave.leaveType === "unpaid" ? "bg-slate-900" : "bg-blue-600"
              }`} />
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs font-black text-slate-700">{new Date(leave.startDate).toLocaleDateString("en-IN", { day: '2-digit', month: 'short', year: 'numeric' })}</span>
                    </div>
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs font-black text-slate-700">{new Date(leave.endDate).toLocaleDateString("en-IN", { day: '2-digit', month: 'short', year: 'numeric' })}</span>
                    </div>
                  </div>
                  <div className="px-3 py-1.5 bg-white border border-slate-100 rounded-xl shadow-sm text-[11px] font-black text-primary-600">
                  ...
                  <div className="pt-2 border-t border-slate-200/50 flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-bold text-slate-500 italic">
                      {leave.leaveType === "unpaid" 
                        ...
                        : `This will deduct ${calculateDays(leave.startDate, leave.endDate)} days from employee balance`}
                    </span>
                  </div>
                )}
              </div>
