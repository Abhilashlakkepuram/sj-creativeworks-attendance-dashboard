...
const StatusBadge = ({ status, missedPunchOut }) => {
  ...
  const base = "inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider shadow-sm border";
  const styles = {
    present: "bg-emerald-50 text-emerald-700 border-emerald-200",
    late: "bg-amber-50 text-amber-700 border-amber-200",
    absent: "bg-rose-50 text-rose-700 border-rose-200",
    "half-day": "bg-blue-50 text-blue-700 border-blue-200",
    "missed punch-out": "bg-orange-50 text-orange-700 border-orange-200",
  };
  ...
  const iconColors = {
    ...
    "missed punch-out": "bg-orange-500",
  };

  return (
    <span className={`${base} ${styles[s] || styles.absent}`}>
      <span className={`w-1.5 h-1.5 rounded-full ${iconColors[s] || iconColors.absent} inline-block shadow-sm`} />
      {s}
    </span>
  );
};
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 font-medium text-slate-700">
                        {d.workHours || (isAbsent ? <span className="text-slate-300">0h 0m</span> : <span className="text-slate-300">—</span>)}
                      </td>
